...
gdzie K jest macierz¹ o wymiarach $\mathrm{(N_u * n_u) \times (N * n_y)}$:
...
Macierz $\mathrm{M}$ o wymiarach $\mathrm{(N * n_y) \times (N_u * n_u)}$ i macierz $\mathrm{Mp}$ o wymiarach $\mathrm{(N * n_y) \times ((D - 1) * n_u)}$ maj¹ postaæ:
...
Wektor $\delta\mathrm{U^P(k)}$ (\ref{dUPm}) zawiera wektory pionowe przesz³ych przyrostów sterowañ d³ugoœci $\mathrm{n_u}$. Z tego powodu wektor $\delta\mathrm{U^P(k)}$ jest d³ugoœci $\mathrm{(D - 1) * n_u}$.
...
Macierz $K$ ma wymiary $\mathrm{n_u \times (N * n_y)}$